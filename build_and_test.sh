#!/bin/bash
# Comprehensive cross-platform build and test script for DiskSense64
...
build_native() {
    ...
    print_status "CLI executable located at: $build_dir/bin/DiskSense.Cli"
    if [ "$BUILD_TOOL" = "Ninja" ]; then
        if [ -f "$build_dir/bin/DiskSense.Gui.exe" ]; then
            print_status "GUI executable located at: $build_dir/bin/DiskSense.Gui.exe"
        elif [ -f "$build_dir/bin/DiskSense.Gui" ]; then
            print_status "GUI executable located at: $build_dir/bin/DiskSense.Gui"
        fi
    else
        if [ -f "$build_dir/bin/DiskSense.Gui.exe" ]; then
            print_status "GUI executable located at: $build_dir/bin/DiskSense.Gui.exe"
...
build_windows() {
    ...
    print_status "CLI executable located at: $build_dir/bin/DiskSense.Cli.exe"
    if [ "$BUILD_TOOL" = "Ninja" ]; then
        if [ -f "$build_dir/bin/DiskSense.Gui.exe" ]; then
            print_status "GUI executable located at: $build_dir/bin/DiskSense.Gui.exe"
        fi
...
build_linux() {
    ...
    print_status "CLI executable located at: $build_dir/bin/DiskSense.Cli"
    if [ "$BUILD_TOOL" = "Ninja" ]; then
        if [ -f "$build_dir/bin/DiskSense.Gui" ]; then
            print_status "GUI executable located at: $build_dir/bin/DiskSense.Gui"
        fi
    else
        if [ -f "$build_dir/bin/DiskSense.Gui" ]; then
...
run_tests() {
    ...
    print_status "Running unit tests ($build_type)..."
...
create_packages() {
    ...
    if [ -d "build-native" ]; then
        ...
        if [ "$(detect_os)" = "Linux" ]; then
            ...
            print_status "Native Linux package created: dist/DiskSense64-native-linux-$(detect_arch).tar.gz"
        else
            zip -r DiskSense64-native-windows-$(detect_arch).zip native
            print_status "Native Windows package created: dist/DiskSense64-native-windows-$(detect_arch).zip"
        fi
        cd ..
    fi
...
main() {
    ...
    print_status "DiskSense64 Cross-Platform Build and Test Script"
    print_status "================================================"
