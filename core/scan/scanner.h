...
struct ScanOptions {
    ...
    std::vector<std::wstring> includeExtensions; // File extensions to include (* = all)
    std::vector<std::wstring> excludeExtensions; // File extensions to exclude
};
...
private:
    ...
    std::vector<uint8_t> computeHeadTailSignature(const std::wstring& path);
    
    // Compute full file hash
    std::vector<uint8_t> computeFullHash(const std::wstring& path);
    
    // Extract file ID from handle
    FileId extractFileId(const std::wstring& path);
    
    // Check if path should be excluded
    bool isExcludedPath(const std::wstring& path, const ScanOptions& options);
    
    // Check file extension filters
    bool matchesExtension(const std::wstring& path, const ScanOptions& options);
};
