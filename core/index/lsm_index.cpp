...
SSTable::SSTable(const std::wstring& filePath)
    : m_filePath(filePath), m_hFile(INVALID_HANDLE_VALUE),
...
void MemTable::put(const FileEntry& entry) {
...
void MemTable::remove(VolumeId volumeId, FileId fileId) {
    entries.emplace_back(volumeId, fileId, FileEntry(), true);
    size += sizeof(Entry);
}

std::optional<FileEntry> MemTable::get(VolumeId volumeId, FileId fileId) const {
...
LsmIndex::LsmIndex(const std::wstring& indexPath, size_t memtableSize)
    ...
    std::string utf8Path = StringUtils::to_utf8_string(indexPath);
...
bool SSTable::load() {
...
#else
    // Open existing file
    int fd = open(utf8Path.c_str(), O_RDONLY);
    if (fd == -1) {
        return false;
    }
    ...
    // Map view of file
    m_mappedView = mmap(nullptr, m_fileSize, PROT_READ, MAP_PRIVATE, fd, 0);
    if (m_mappedView == MAP_FAILED) {
        close(fd);
        return false;
    }
    
    // Close file descriptor (we keep the mapping)
    close(fd);
#endif
...
bool SSTable::save(const MemTable& memTable) {
...
#else
    // Create file
    int fd = open(utf8Path.c_str(), O_RDWR | O_CREAT | O_TRUNC, 0644);
    if (fd == -1) {
        return false;
    }
    ...
    // Map view of file
    m_mappedView = mmap(nullptr, fileSize, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if (m_mappedView == MAP_FAILED) {
        close(fd);
        return false;
    }
...
std::wstring LsmIndex::getMemTableFileName() const {
    return m_indexPath + std::wstring(PATH_SEPARATOR_STR) + std::wstring(L"memtable.dat");
}
...
#endif
    return m_indexPath + std::wstring(PATH_SEPARATOR_STR) + std::wstring(fileName);
}
